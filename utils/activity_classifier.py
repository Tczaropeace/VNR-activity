...
def load_activity_classifier():
    ...
    try:
        ...
        # Removed: st.info("Loading activity classification model...")
        
        if not model_path.exists():
            # Removed: st.error(f"Model directory not found: {model_path}")
        ...
        if missing_files:
            # Removed: st.error(f"Missing model files: {missing_files}")
        ...
        # Removed: st.success("Activity classification model loaded successfully!")
        ...
        # Removed: st.error(f"{error_msg}")
        logger.error(error_msg)
        return None, None, False

def classify_sentences(sentences_data: List[Dict[str, Any]], 
                      model, tokenizer, 
                      batch_size: int = 16) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    ...
    Returns:
        Tuple of (List of sentence dictionaries with added 'activity_prediction' field,
                  Progress info dict with 'total_batches' and 'current_batch')
    """
    if not sentences_data:
        return [], {'total_batches': 0, 'current_batch': 0}
    ...
    try:
        ...
        # Removed UI elements - just process the data
        ...
        for i in range(0, len(enhanced_texts), batch_size):
            ...
            # Removed: status_text.text(f"Classifying batch {batch_num}/{total_batches} ({len(batch_texts)} sentences)")
        ...
        # Removed: Clear progress indicators
        # progress_bar.empty()
        # status_text.empty()
        ...
        # Removed: st.error(f"Classification failed: {str(e)}")
        logger.error(f"Classification error: {str(e)}")
        
        # Return original data with no predictions on error
        return [dict(sent_data, activity_prediction=0) for sent_data in sentences_data], {'error': str(e)}
